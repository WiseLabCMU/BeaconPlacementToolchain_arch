function Call_BeaconPlacement(COMPARE_METRIC)
...
F_NewFig=1;
%[ClustData, ClustCnt]=ClusterPointsInFloorPlan(FloorPlan_Path,F_NewFig);
...
%ClustDataOrig = ClustData;
% Plot cluster and indicate the cluster number ('1' is largest cluster)
    ...
    if(ClustData{IndClust,5}==0) % the cluster is not covered by any beacons yet
        ...
        for i = 1:size(CornerInLosInd,1) 
            ...
            for config = 1:length(PotentialBeacLoc)
                ...
                ClassTemp=GetClassOfPoints(PtsInFp_LosBeac,BeaconPos,FloorPlan_Path);
                StoreClass(:,config) = ClassTemp;
                %PlotCoverageClass( BeaconPos,ClassTemp,FloorPlan_Path,F_color );
                
                if COMPARE_METRIC=='D'
                    [DOP,DOP_UL]=GetDopOfPoints(PtsInFp_LosBeac,ClassTemp,FloorPlan_Path);
                    F_color=1;
...
if(COMPARE_METRIC=='D')
    ResultPath = fullfile(FloorPlan_Path,'BeacPlacement_D');
else
    ResultPath = fullfile(FloorPlan_Path,'BeacPlacement_U');
end
